            ...
            <table class="table table-striped grid-view-tbl">
                <thead>
                    <tr class="header-row">
                        ...
                        <th width="10%"><a href="{{route('admin.nourriture.index')}}" class="text-info">
                            @if(Request::is('admin/nourriture/*'))
                                <i class="fa fa-close"></i> Réinitialiser le filtre</a>
                            @endif
                        </th>
                    </tr>
                    <tr class="search-row">
                        <form class="search-form">
                            ...
                            <td style="min-width: 6em;">
                                <button type="submit" class="fa fa-search form-control btn btn-primary"> Rechercher</button>
                            </td>
                        </form>
                    </tr>
                </thead>
            ...
            <div class="col-md-3">
                <div class="ibox">
                    <div class="ibox-content product-box">
                        ...
                        <div class="{{$record->media->count() > 0 ? '' : 'product-imitation'}}">
                            @if($record->media->count() > 0)
                                <img class="img-fluid" src="{{$record->getImageUrl('medium')}}" alt="{{$record->libelle}}">
                            @else
                                <img src="http://placehold.it/374x200" alt="{{$record->libelle}}">
                            @endif
                        </div>
                        <div class="product-desc">
                                <span class="product-price">
                                    {{ $record->cout() }} / <small><i>{{$record->coutFmg()}}</i></small>
                                </span>
                            <small class="text-muted">
                                {{ $record->category ? $record->category->libelle : '' }} / {{ $record->sousCategory ? $record->sousCategory->libelle : '' }}
                            </small>
                            <a href="{{route('admin.nourriture.index')}}/{{$record->id}}" class="product-name"> {{ $record->libelle }}</a>



                            <div class="small m-t-xs">
                                {!! str_limit($record->description, 50, '...') !!}
                            </div>
                            <div class="m-t text-righ">

                                <a href="{{route('admin.nourriture.index')}}/{{$record->id}}" class="btn btn-xs btn-outline btn-primary">Info <i class="fa fa-long-arrow-right"></i> </a>
                            </div>
                        </div>
                    </div>
                </div>
